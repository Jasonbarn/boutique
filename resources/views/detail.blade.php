        ...
        <div class="container px-6 py-8 mx-auto">
            <div class="lg:flex lg:-mx-2">
                <div class="space-y-3 lg:w-1/5 lg:px-2 lg:space-y-4">
                    <a href="{{ route('products.index') }}" class="block font-medium text-gray-500 dark:text-gray-300 hover:underline">Back to Products</a>
                </div>

                <div class="mt-6 lg:mt-0 lg:px-2 lg:w-4/5">
                    @isset($product)
                        <div class="flex items-center justify-between text-sm tracking-widest uppercase">
                            <p class="text-gray-500 dark:text-gray-300">{{ $product->name }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            <div class="flex flex-col items-center justify-center w-full max-w-lg mx-auto">
                                <img class="object-cover w-full rounded-md h-72 xl:h-80" src="{{ $product->image_url }}" alt="{{ $product->name }}">
                                <h4 class="mt-2 text-lg font-medium text-gray-700 dark:text-gray-200">{{ $product->name }}</h4>
                                <p class="text-blue-500">${{ $product->price }}</p> 
                                <p class="text-gray-500 dark:text-gray-300">Description: {{ $product->description }}</p>
                                <p class="text-gray-500 dark:text-gray-300">Stock: {{ $product->stock }}</p>
